...
document.querySelector('#check-close-button').onchange = function (e) {
    ...
    var asideLeftContent = document.querySelector('.aside-left-content')

    asideLeft.classList.toggle('display-none')
    asideLeftContent.classList.toggle('display-none')
    asideLeft.classList.toggle('w50px-h50px')
...
function loadBallList(array) {
    ...
    var numberListBox = document.querySelector('.number-list-box')

    if (listOfBall == `<div class="number-ball"></div>`) {
        listOfBall = ''
    }

    numberListBox.innerHTML = listOfBall
}
...
function onePage(page) {
    var radioPage = document.querySelector('.radio-page')
    var checkAreaInput = document.querySelector('.check-area')
    var bingoBox1 = document.querySelector('#bingo-box1')
    var bingoBox2 = document.querySelector('#bingo-box2')
    var bingoBox = document.querySelector('#bingo-box')

    radioPage.style.display = 'flex'
    checkAreaInput.classList.add('mr-0px')

    if (page == 1) {
        bingoBox1.style.display = 'none'
        bingoBox2.style.display = 'block'
    } else {
        bingoBox1.style.display = 'block'
        bingoBox2.style.display = 'none'
    }
    bingoBox.style.width = '300px'
}
...
function twoPage() {
    ...
    checkAreaInput.classList.remove('mr-0px')
    bingoBox1.style.display = 'block'
    bingoBox2.style.display = 'block'
...
function checkBingo(arr) {
    ...
    for (var key in obj) {
        ...
        for (var num = 0; num < obj[key].length; num += 5) {
            ...
            if (typeof results == 'object' && results.length == 3) {
                bingoHtml = bingoHtml + `<div class="number-ball" style="background-color: ${results[2]}; border: solid 4px #fff"></div>`
            } else if (typeof results == 'object' && results != '') {
                var kt = true
    ...
    message.innerHTML = html + bingoHtml
...
function check5Number(key, num1, num2, num3, num4, num5) {
    ...
    if (array.includes(num1)) count++
    if (array.includes(num2)) count++
    if (array.includes(num3)) count++
    if (array.includes(num4)) count++
    if (array.includes(num5)) count++

    if (count == 5) return ['bingo', '', key]

    if (count == 4) {
        if (!array.includes(num1)) return ([key, num1])
        if (!array.includes(num2)) return ([key, num2])
        if (!array.includes(num3)) return ([key, num3])
        if (!array.includes(num4)) return ([key, num4])
        if (!array.includes(num5)) return ([key, num5])
    }
